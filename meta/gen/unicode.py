...
UNICODE_PROPERTIES = [
    ...
    UnicodeProperty(
        "Default_Ignorable_Code_Point", "DerivedCoreProperties.txt", "No", True
    ),
...
def main():
    ...
    for prop in UNICODE_PROPERTIES:
        ...
        if prop.name == "General_Category":
            ...
            prop.values_table.append(
                UnicodePropertyRange([CodePointRange(0xFDD0, 0xFDEF + 1)], "Invalid")
            )
            for s in range(0xFFFE, 0x10FFFE, 0x10000):
                prop.values_table.append(
                    UnicodePropertyRange([CodePointRange(s, s + 2)], "Invalid")
                )
